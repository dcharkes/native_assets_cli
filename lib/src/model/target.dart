...
class OS {
  ...
  String libraryFileName(String name, Packaging packaging) {
    switch (packaging) {
      case Packaging.dynamic:
        return dylibFileName(name);
      case Packaging.static:
        return staticlibFileName(name);
    }
    throw StateError("Unknown version from Packaging '$packaging'.");
  }
...
class Target implements Comparable<Target> {
  ...
  /// Target for [os] and [architecture].
  ///
  /// Must be included in [Target.values], which can be checked with [isValid].
  factory Target(OS os, Architecture architecture) =>
      _canonicalizedTargets[os]![architecture]!;
  ...
  static final Map<OS, Map<Architecture, Target>> _canonicalizedTargets = {
    for (OS os in OS.values)
      os: {
        for (Architecture architecture in Architecture.values)
          if (Target.values
              .where((target) =>
                  target.os == os && target.architecture == architecture)
              .isNotEmpty)
            architecture: Target.values.firstWhere((target) =>
                target.os == os && target.architecture == architecture),
      }
  };
  ...
  @override
  String toString() => '${_osString}_$_architectureString';
